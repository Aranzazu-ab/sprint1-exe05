...
The method `getSortedContent()` allows the user to list the content of a directory in alphabetical order. With
`readFilesRecursive()` the application can explore subdirectories and return all files and folders 
sorted alphabetically. It also shows if the element is a directory or a file, including its last modification 
date.
The program also saves the directory tree information into a TXT file instead of only showing it on the screen `saveFilesRecursive()`.
And `readTxtFile()` adds the possibility to read any TXT file and return its content. The content can also be printed 
using a separate method. `serializeObject()` and `deserializeObject()` methods implements object serialization. 
A Java object can be converted into a `.ser` file and later restored using deserialization.

During development, unit tests were created to check the main behaviors:
...
## CONCLUSION
This project was a good exercise to practice file handling in Java. Building the `FileManager` class helped 
to understand how to read files and folders, check if a path is valid, and work with recursive methods to 
explore directory trees.
It was also useful to learn how to save information into TXT files and how object serialization works, converting 
a Java object into a `.ser` file and then restoring it back with deserialization.

Writing the unit tests in `FileManagerTest` helped to confirm that the methods worked correctly, especially 
for cases like invalid directories or wrong file types, and to make sure the alphabetical sorting and 
recursive reading worked as expected.

# LEVEL 2 PROPERTIES
This Java application modifies the previous file management exercise by moving the configuration from the code 
into an external `.properties` file.
The goal is to separate the application configuration from the program logic, making the application easier to 
modify and reuse in different environments.
...
## CONCLUSIONS
Using a `.properties` file makes the application more flexible because paths and settings can be changed 
without recompiling the code. For example, another directory or output file can be used only by editing 
`configuration.properties`.